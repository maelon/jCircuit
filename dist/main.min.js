...
var utils = {
    ...
    MixinInterface: function MixinInterface(cls, itf) {
        var proto = {};
        var protoKeys = Object.getOwnPropertyNames(itf.prototype);
        protoKeys.forEach(function (key) {
            if (key !== 'constructor') {
                proto[key] = itf.prototype[key];
            }
        });

        var NewCls = function NewCls() {
            _classCallCheck(this, NewCls);
        };

        NewCls.prototype = Object.create(cls.prototype, proto);
        return NewCls;
    }
};
...
var _interface3 = __webpack_require__(3);
...
var Line = function (_ILine) {
    _inherits(Line, _ILine);
    ...
    _createClass(Line, [{
            ...
            if (['on', 'off'].includes(flag)) {
                if (flag != this._switch_state) {
                    if (this._switch_state === 'on') {
                        this._processElement(this, data).catch(function (error) {
        ...
        */
        ...
        value: function process() {
            ...
            var _loop = function _loop(i) {
                elements_promise = elements_promise.then(function (ret, name) {
                    if (ret === _index2.default.LINE_SWITCH_OFF || _this2._switch_state === 'off') {
                        return Promise.resolve(_index2.default.LINE_SWITCH_OFF);
                    }
                    if (ret === _index2.default.CIRCUIT_BREAK) {
                        return Promise.resolve(ret, name || elements[i].elementName);
                    }
                    return _this2._processElement(elements[i], ret);
                });
